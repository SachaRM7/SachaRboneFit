...
export default function PageBienvenue() {
  ...
  return <div className="relative h-dvh overflow-hidden bg-papier text-encre">
    ...
    <div className="relative mx-auto flex h-full max-w-[520px] flex-col">
      ...
      <main className="min-h-0 flex-1 overflow-y-auto px-5" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 7.5rem)" }}>
        ...
        {etape === 1 && <EtapeSection titre="Les repères utiles" sousTitre="Ils servent à cadrer les estimations. Ton poids reste une mesure datée."><QuiTuEs valeurs={mesures} onChange={setMesures} requis erreurs={erreursCorps} /></EtapeSection>}
      ...
      <footer className="absolute inset-x-0 bottom-0 z-30 border-t border-filet bg-papier/90 px-5 pt-3 backdrop-blur-xl" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + .85rem)" }}>{(raisonBlocage || erreur) && <p className="mb-2 text-center text-xs font-medium text-perte" role="alert">{erreur ?? raisonBlocage}</p>}<button type="button" onClick={avancer} disabled={Boolean(raisonBlocage) || envoi} className="flex min-h-14 w-full items-center justify-center gap-2 rounded-[18px] bg-encre px-5 font-semibold text-papier shadow-[0_10px_25px_rgba(20,30,25,.16)] transition-transform active:scale-[.99] disabled:cursor-not-allowed disabled:opacity-35">{envoi ? "Création en cours…" : etape === DERNIERE ? "Créer mon programme" : etape === 0 ? "Commencer" : "Continuer"}<ArrowRight className="size-5" /></button></footer>
    </div>
  </div>;
}
