...
import { suggestionsAccueilCoach } from "@/lib/coach/accueil-conversation";
export async function POST(request: Request) {
  const userId = await getAuthenticatedUserId();
  if (!userId) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const body = await request.json().catch(() => null);
  try {
    const contexte = contexteValide(body?.contexte) ?? { ecran: "accueil" as const };
    const [profil, ecran] = await Promise.all([
      loadCoachContext(userId),
      resoudreContexte(userId, contexte),
    ]);
    return NextResponse.json({
      ...
      suggestions: suggestionsAccueilCoach({ contexte, seance: Boolean(ecran.refs?.seanceTemplateId || ecran.refs?.sessionLogId),
